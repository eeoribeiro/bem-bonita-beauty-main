    ...
    const nextCart = (() => {
      ...
      if (existing) {
        return currentCart.map((item) =>
          cartItemKey(item) === cartItemKey({ id: product.id, optionId })
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {relatedProducts.map((item) => (
                  <Link
                    ...
                    className="rounded-3xl border border-border bg-card p-3 shadow-card transition hover:-translate-y-1 hover:border-primary/50"
                  >
